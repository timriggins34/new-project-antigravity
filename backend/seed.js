...
async function main() {
  console.log('🌱 Starting database seed (Normalized & Expanded)...');
  ...
  // 1. CLIENTS (with Relational Documents)
  const client1 = await prisma.client.create({
    data: {
      ...
      documents: {
        create: [
          { name: 'IEC Certificate', url: 'https://storage.tradeflow.in/docs/iec_001.pdf' },
          { name: 'GST Registration', url: 'https://storage.tradeflow.in/docs/gst_001.pdf' }
        ]
      }
    }
  });
  ...
  const client2 = await prisma.client.create({
    data: {
      ...
      documents: {
        create: [
          { name: 'PAN Card Copy', url: 'https://storage.tradeflow.in/docs/pan_002.pdf' }
        ]
      }
    }
  });
  console.log('✅ Seeded clients and documents');

  // 2. VENDORS
  const vendor1 = await prisma.vendor.create({
    data: { vendor_id: 'VND-001', name: 'FastTrack Logistics Pvt Ltd', type: 'Trucking', contact: 'Ramesh Singh', phone: '+91 98765 43210', email: 'ops@fasttrack.in' }
  });
  const vendor2 = await prisma.vendor.create({
    data: { vendor_id: 'VND-002', name: 'Oceanic Freight Solutions', type: 'Freight', contact: 'Sarah Miller', phone: '+65 6225 0198', email: 'smiller@oceanic.com' }
  });
  console.log('✅ Seeded vendors');

  // 3. CLEARANCE JOBS (with HS Code Items & Duty Tracking)
  await prisma.clearanceJob.create({
    data: {
      ...
      hsCodeItems: {
        create: [
          { itemName: 'Central Processing Units', hsCode: '84713010', assessableValue: 500000.00 },
          { itemName: 'Graphics Storage Units', hsCode: '84717020', assessableValue: 200000.00 }
        ]
      },
  ...
  await prisma.clearanceJob.create({
    data: {
      ...
      hsCodeItems: {
        create: [
          { itemName: 'Hydraulic Valves', hsCode: '84812000', assessableValue: 85000.00 }
        ]
      },
  ...
  // 4. DOC JOBS (with relational documents and checklist flags)
  ...
  await prisma.licence.create({
    data: { licence_id: 'EPCG-2023-45', client: 'Alpha Manufacturing Ltd', type: 'EPCG', status: 'Active', expiry: '2027-10-15', utilized: 45, alert: false }
  });

  await prisma.logisticsTrip.create({
    data: { trip_id: 'TRP-501', job: 'IMP-8802', truck: 'MH-04-EY-1234', driver: 'Suresh Patil', from: 'JNPT Port', to: 'Vashi Warehouse', status: 'dispatch', eta: 'Today, 18:00', delayed: false }
  });

  console.log('\n🎉 Database re-seeded successfully with expanded relational data!');
}
